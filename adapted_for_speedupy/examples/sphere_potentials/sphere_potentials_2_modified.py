...
def f3(S, plates, betaDeltaG0, hArr, L):
    ...
    return (plates, betaFPlate, {filename: data})
...
def f4(S, L, plates, ALPHA, ALPHA_P, hArr):
    ...
    for betaDeltaG0 in np.arange(-12, 1, 0.5):
        (plates, betaFPlate, r2) = f3(S, plates, betaDeltaG0, hArr, L)
        result2.update(r2)
        result.update(f2(S, betaDeltaG0, hArr, betaFPlate, L))
    return (plates, result, result2)

@deterministic
def f5(L, plates, ALPHA, ALPHA_P, hArr):
    result = {}
    result2 = {}
    for S in np.arange(0.1, 1.01, 0.04):
        (plates, r, r2) = f4(S, L, plates, ALPHA, ALPHA_P, hArr)
        result.update(r)
        result2.update(r2)
    return (plates, result, result2)
...
def main():
    ...
    (plates, result, result2) = f5(L, plates, ALPHA, ALPHA_P, hArr)
